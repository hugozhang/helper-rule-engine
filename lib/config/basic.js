...
var ProximityOperator = Operators.ProximityOperator;
...
var conjunctions = {
  AND: {
    ...
    spelConjs: ["and", "&&"],
    reversedConj: "OR",
    formatConj: function formatConj(children, conj, not, isForDisplay, mode) {
      var _conj = children.size > 1 ? (not ? "NOT " : "") + (mode == "array" ? "" : "(") + children.join(" " + (isForDisplay ? "AND" : "&&") + " ") + (mode == "array" ? "" : ")") : (not ? "NOT (" : "") + children.first() + (not ? ")" : "");
...
var stringifyForDisplay = exports.stringifyForDisplay = function stringifyForDisplay(v) {
...
var widgets = {
  ...
  date: {
    ...
    spelFormatValue: function spelFormatValue(val, fieldDef, wgtDef, op, opDef) {
      ...
      return "string_to_timestamp('".concat(dateVal.format("YYYY-MM-DD"), "', 'yyyy-MM-dd')");
    },
  ...
  time: {
    ...
    spelFormatValue: function spelFormatValue(val, fieldDef, wgtDef, op, opDef) {
      ...
      return "string_to_timestamp('".concat(dateVal.format("HH:mm:ss"), "', 'HH:mm:ss')");
    },
    jsonLogic: function jsonLogic(val, fieldDef, wgtDef) {
  ...
  datetime: {
    ...
    spelFormatValue: function spelFormatValue(val, fieldDef, wgtDef, op, opDef) {
      ...
      return "string_to_timestamp('".concat(dateVal.format("YYYY-MM-DD HH:mm:ss"), "', 'yyyy-MM-dd HH:mm:ss')");
    },
    jsonLogic: function jsonLogic(val, fieldDef, wgtDef) {
      return (0, _moment["default"])(val, wgtDef.valueFormat).toDate();
    },
...
var types = {
  text: {
    ...
    widgets: {
      text: {
        operators: ["single_equal", "equal", "not_equal", "like", "not_like", "starts_with", "ends_with", "proximity", "is_empty", "is_not_empty", "is_null", "is_not_null"],
        widgetProps: {},
        opProps: {}
      },
      textarea: {
        operators: ["single_equal", "equal", "not_equal", "like", "not_like", "starts_with", "ends_with", "is_empty", "is_not_empty", "is_null", "is_not_null"],
        widgetProps: {},
        opProps: {}
      },
      field: {
        ...
        "single_equal", "equal", "not_equal", "proximity" //can exclude if you want
        ]
      }
    }
  },
  number: {
    ...
    widgets: {
      number: {
        operators: ["single_equal", "equal", "not_equal", "less", "less_or_equal", "greater", "greater_or_equal", "between", "not_between",
        // "is_empty",
        // "is_not_empty",
        "is_null", "is_not_null"]
      },
      slider: {
        operators: ["single_equal", "equal", "not_equal", "less", "less_or_equal", "greater", "greater_or_equal",
        // "is_empty",
        // "is_not_empty",
        "is_null", "is_not_null"]
      }
  ...
  "boolean": {
    ...
    widgets: {
      "boolean": {
        operators: ["single_equal", "equal", "not_equal", "is_null", "is_not_null"],
        widgetProps: {
      ...
      field: {
        operators: ["single_equal", "equal", "not_equal"]
      }
    }
  },
  "!group": {
    ...
    widgets: {
      number: {
        ...
        "some", "all", "none",
        // w/ operand - count
        "single_equal", "equal", "not_equal", "less", "less_or_equal", "greater", "greater_or_equal", "between", "not_between"],
        opProps: {
          equal: {
            label: "Count =="
          },
...
var settings = _objectSpread(_objectSpread({}, _default2.settings), {}, {
  ...
  formatAggr: function formatAggr(whereStr, aggrField, operator, value, valueSrc, valueType, opDef, operatorOptions, isForDisplay, aggrFieldDef) {
    ...
    if (cardinality == 0) {
      var forIn = labelForFormat.replace(" ", " $" + aggrField.slice(0, aggrField.length - 1) + " ") + " " + aggrField;
      var cond = whereStr ? "".concat(whereStr) : "";
      return "".concat(forIn, " IF ").concat(cond);
      // const cond = whereStr ? ` HAVE ${whereStr}` : "";
      // return `${labelForFormat} OF ${aggrField}${cond}`;
    } else if (cardinality == undefined || cardinality == 1) {
      var _cond = whereStr ? " IF ".concat(whereStr) : "";
