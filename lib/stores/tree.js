...
var _addChildren1 = function _addChildren1(config, item, children) {
  if (children && Array.isArray(children)) {
    item.children1 = new _immutable["default"].OrderedMap(children.reduce(function (map, it) {
      ...
      return _objectSpread(_objectSpread({}, map), {}, _defineProperty({}, id1, new _immutable["default"].Map(it1)));
    }, {}));
  }
};
...
var addItem = function addItem(state, path, type, id, properties, config) {
  ...
  if (canAdd) {
    var newChildren = new _immutable["default"].OrderedMap(_defineProperty({}, id, new _immutable["default"].Map(item)));
    if (!hasChildren) {
      state = state.setIn(childrenPath, newChildren);
    } else if (isLastDefaultCase) {
      var _Immutable$OrderedMap2;
      var last = targetChildren.last();
      var newChildrenWithLast = new _immutable["default"].OrderedMap((_Immutable$OrderedMap2 = {}, _defineProperty(_Immutable$OrderedMap2, id, new _immutable["default"].Map(item)), _defineProperty(_Immutable$OrderedMap2, last.get("id"), last), _Immutable$OrderedMap2));
